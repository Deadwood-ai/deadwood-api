...
def calculate_cog(tiff_file_path, cog_target_path, profile="webp", overviews=None):
    ...
    #use_cog_driver=, # Use GDAL COG driver if set to True. COG driver is available starting with GDAL 3.1.
    )

    if not cog_validate(cog_target_path):
        # wenn die settings dazu führen könnten dass das cog kaputt ist(?):
        raise
    
    # zum testen und nachvollziehen:
    info_output_cog = cog_info(cog_target_path)

    print(info_output_cog['Profile'])
    print(info_output_cog['GEO'])
    print(info_output_cog['Tags'])
    print(f'Anzahl overviews: {len(info_output_cog['IFD']) - 1}')
    for level in info_output_cog['IFD']:
        print(level)
