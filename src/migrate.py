...
def migrate_file(old_metadata: dict) -> None:
    ...
    try:
        ...
        logger.exception(f"An error occurred while trying to upload the dataset: {str(e)}", extra={'token': conf['token']})
        return
    ...
    logger.info(f"Migration for new Dataset <ID={dataset.id}> successfull after {t2 - t1} seconds.", extra={'token': conf['token']})
