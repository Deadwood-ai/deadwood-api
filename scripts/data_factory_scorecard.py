...
def render_manual(payload: dict[str, Any]) -> str:
	lines = [
		...
		'Use the supported aggregates in docs/playbooks/platform-status-check.md and docs/playbooks/operator-data-coverage.md.',
		'The full scorecard needs download-grant extra and v2_full_dataset_view, which the documented analyst and monitor grants do not provide.',
		'Keep that coverage gap explicit; do not broaden grants or substitute a privileged connection.',
		'Print the current SQL for inspection only:',
		'',
		'```bash',
